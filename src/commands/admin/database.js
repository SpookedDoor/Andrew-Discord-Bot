        ...
        try {
            ...
            if (focusedOption.name === "category") {
                ...
                const payload = choices.map((choice) => {
                    const name =
                        choice.length > MAX_LEN ? choice.slice(0, MAX_LEN - 3) + "..." : choice;
        ...
        try {
            ...
            if (subcommand === "add") {
                ...
                if (categories.rows.length === 0)
                    return interaction.reply({
                        content: `Category "${category}" does not exist. Please create it first with SQL.`,
                        flags: MessageFlags.Ephemeral,
                    });

                const { rows: existingMessages } = await db.query(
                    `SELECT id FROM messages WHERE category_id = $1 AND content = $2`,
                    [categories.rows[0].id, message],
                );
                if (existingMessages.length > 0)
                    return interaction.reply({
                        content: `Message "${message}" already exists in category "${category}".`,
                        flags: MessageFlags.Ephemeral,
                    });

                const categoryId = categories.rows[0].id;
                const insertMessage = await db.query(
                ...
                if (attachment) {
                    ...
                    console.log(
                        `${interaction.user.username} added message "${message}" with attachment "${attachment.url}" to category "${category}".`,
                    );
                    return interaction.reply({
                        content: `Added message "${message}" with attachment "${attachment.url}" to category "${category}".`,
                        flags: MessageFlags.Ephemeral,
                    });
                } else {
                    console.log(
                        `${interaction.user.username} added message "${message}" to category "${category}".`,
                    );
                    return interaction.reply({
                        content: `Added message "${message}" to category "${category}".`,
                        flags: MessageFlags.Ephemeral,
                    });
                }
                ...
                const messages = await db.query(
                    `SELECT id FROM messages WHERE category_id = $1 AND content = $2 LIMIT 1`,
                    [categoryId, message],
                );
                if (messages.rows.length === 0)
                    return interaction.reply({
                        content: `Message "${message}" does not exist in category "${category}".`,
                        flags: MessageFlags.Ephemeral,
                    });
                ...
                console.log(
                    `${interaction.user.username} removed message "${message}" from category "${category}".`,
                );
                return interaction.reply({
                    content: `Removed message "${message}" from category "${category}".`,
                    flags: MessageFlags.Ephemeral,
                });
            }
        } catch (error) {
